...
`.github/workflows/build-release.yml` builds a PyInstaller **onedir** bundle (`dist/SST39FlashProgrammer/`), zips it to `SST39FlashProgrammer.zip` (Python 3.12, Windows runner), and uploads the zip. Triggers: every push to `main` touching non-doc files, plus `workflow_dispatch`. Onedir was chosen over onefile because the onefile bootloader extracts to `%TEMP%` on every launch (5-10 s cold start); onedir starts in ~1 s. Each build creates an `Auto build build-<ts>-<sha>` release marked Latest; the prune step keeps the zip (and any legacy `.exe`) only on the 4 newest auto-builds (older release notes/tags survive). Manually-tagged semver releases (`v0.1.0`-style) are untouched by the prune.
