...
- `binFileTransferGui.py` — **PySide6/Qt** GUI front-end (rewritten from Tkinter). Dark left sidebar nav + light card-based content area, blue accent. Four pages: `燒錄 ROM` (flash), `GPIO 設定` (manual pin poker, persistent connection), `TDBG` (pin picker + three one-click preset waveforms TDBG1/2/3 via `TDBG_PRESET`), `波形錄製` (live multi-pin recorder). Split across sibling modules: `gui_theme.py` (QSS stylesheet + the `PALETTE` colour dict), `gui_widgets.py` (`Sidebar` / `Card` / `LogPane` / `PinGrid` / QPainter `WaveformView`), `gui_pages.py` (the four page classes + the threaded `Worker`), and `gui_data.py` (carried-over flash-pin constants, `TDBG_PIN_LABELS`, and the `format_us` duration helper). `binFileTransferGui.py` itself is the `QMainWindow` + the shared-serial connection manager. `binFileTransfer_core.py` is unchanged by the GUI rewrite.
